...
document.addEventListener("DOMContentLoaded", () => {
    ...
    registerForm.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        if (response.ok && result.status === "success") {
          ...
          localStorage.setItem("user_session", JSON.stringify({ id: result.user_id, email: result.email }));
  ...
  function showToast(type, message) {
    ...
    setTimeout(() => {
      toast.style.animation = "slideOutRight 0.4s cubic-bezier(0.34, 1.56, 0.64, 1) forwards";
