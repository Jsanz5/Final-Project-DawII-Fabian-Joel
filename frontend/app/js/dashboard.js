...
function renderList(target, items, limit = 5) {
  ...
  const limited =
    limit === null || limit === undefined ? list : list.slice(0, limit);
...
function renderHistoryDetail(audit) {
  ...
  if (metaEl) {
    const urlLabel = audit?.url
      ? `URL: ${simplifyHistoryUrl(audit.url)}`
      : "URL no disponible";
    const dateLabel = audit?.created_at
      ? `Fecha: ${formatHistoryDate(audit.created_at)}`
      : "Fecha no disponible";
...
function initWarpCanvas() {
  ...
  const draw = () => {
    ...
    const points = 240,
      maxWarp = state.radius * 0.15,
      lineWidth = state.radius * 0.35;
    let gradient;
    ...
    for (let i = 0; i <= points; i++) {
      ...
      if (state.hasMouse) {
        const dx = baseX - state.mouseX,
          dy = baseY - state.mouseY;
        const dist = Math.sqrt(dx * dx + dy * dy);
        const influence = Math.max(0, 1 - dist / (state.radius * 1.5));
        offset = influence * maxWarp;
      }
      const wave =
        (Math.sin(state.tick * 0.015 + angle * 3) * 0.4 +
          Math.cos(state.tick * 0.02 + angle * 4) * 0.3 +
          Math.sin(state.tick * 0.01 + angle * 2) * 0.3) *
        (state.radius * 0.12);
...
function bindContentForm() {
  ...
  chipGroup?.querySelectorAll(".content-chip").forEach((chip) => {
    chip.addEventListener("click", () =>
      setActiveChip(chip.dataset.value || "articulo"),
    );
  });
...
function bodyTextToHtml(text) {
  return text
    ...
    .map(
      (p) =>
        `<p style="margin:0 0 8px 0;line-height:1.6;color:#1f2937;font-size:12px">${p.replace(/\n/g, "<br>")}</p>`,
    )
    .join("");
}
...
function bindContentExport() {
  ...
  btn.addEventListener("click", async () => {
    ...
    try {
      const getText = (id) =>
        document.getElementById(id)?.innerText.trim() || "N/A";
      ...
      container.style.cssText =
        "position:fixed;left:-9999px;top:0;width:794px;background:white;font-family:Arial,Helvetica,sans-serif;font-size:12px;line-height:1.4;color:#111827;z-index:-1";
...
function renderAnalyzeResults(data) {
  ...
  const analysisProps = analy.analysis || {};
  ...
  const competitorCards = comps.length
    ? comps
        ...
        <div class="results-island analyze-competitor-card">
          ...
          ${c.meta_description ? `<p class="analyze-competitor-meta" style="margin-top: 6px; font-size: 11px;"><strong>Meta:</strong> ${c.meta_description}</p>` : ""}
          ${c.preview || c.snippet ? `<p class="analyze-competitor-snippet" style="margin-top: 4px; font-size: 11px;"><strong>Preview:</strong> ${c.preview || c.snippet}</p>` : ""}
          ${
            c.h1 && c.h1.length
              ? `<div class="competitor-heading-list" style="margin-top: 6px; font-size: 11px;"><strong>H1s:</strong> <ul style="margin: 2px 0 0 16px; padding: 0;">${c.h1
                  .filter((h) => h.trim() !== "")
                  .map((h) => `<li>${h}</li>`)
                  .join("")}</ul></div>`
              : ""
          ...
          ${
            c.h2 && c.h2.length
              ? `<div class="competitor-heading-list" style="margin-top: 6px; font-size: 11px;"><strong>H2s:</strong> <ul style="margin: 2px 0 0 16px; padding: 0;">${c.h2
                  .filter((h) => h.trim() !== "")
                  .map((h) => `<li>${h}</li>`)
                  .join("")}</ul></div>`
              : ""
    ...
    <div class="analyze-results-wrap">
      ...
      <div class="results-islands">
        ...
        <div class="results-island analyze-intent">
          <div class="island-header">
            <div class="island-title"><i class="ri-thumb-up-line"></i> Fortalezas</div>
          </div>
          ${copyableField("resAnalyzeFortaleza", analysisProps.fortaleza || analy.search_intent, "analyze-intent-copy")}
        </div>

        <div class="results-island analyze-gap">
          <div class="island-header">
            <div class="island-title"><i class="ri-lightbulb-line"></i> Oportunidades</div>
          </div>
          ${copyableField("resAnalyzeOportunidad", analysisProps.oportunidad || analy.content_gap, "analyze-gap-copy")}
        </div>
        ...
        <div class="results-island results-island-wide analyze-meta-island">
          <div class="island-header">
            <div class="island-title"><i class="ri-file-text-line"></i> Meta Description Sugerida</div>
          </div>
          ${copyableField("resAnalyzeMeta", analy.meta_description, "analyze-meta-copy")}
        </div>

        <div class="results-island analyze-keywords-island">
          <div class="island-header">
            <div class="island-title"><i class="ri-price-tag-3-line"></i> Keywords Target</div>
          </div>
          <div class="analyze-keywords-list" id="resAnalyzeKeywords" style="display: flex; flex-wrap: wrap; gap: 4px; padding: 12px 14px 14px 14px; background: #f9fafb; border: 1px solid #e5e7eb; border-radius: 7px; margin-top: 6px;">
            ${
              analy.keywords_target && analy.keywords_target.length
                ? analy.keywords_target
                    .map(
                      (k) =>
                        `<span class="content-chip" style="background:#e0e7ff; color:#4338ca; border:none; padding:4px 8px; font-size:11px;">${k}</span>`,
                    )
                    .join("")
        ...
        <div class="results-island analyze-links-island">
          <div class="island-header">
            <div class="island-title"><i class="ri-links-line"></i> Enlaces Internos</div>
          </div>
          <ul id="resAnalyzeLinks" class="analyze-h2-list" style="margin-top: 8px;">
            ${
              analy.internal_links && analy.internal_links.length
                ? analy.internal_links
                    .map(
                      (l) =>
                        `<li class="analyze-h2-item"><span>${l}</span></li>`,
                    )
                    .join("")
                : `<li class="analyze-h2-item"><span>Sin enlaces</span></li>`
            }
          </ul>
        </div>
        ...
        <div class="results-island results-island-wide analyze-schema-island">
          <div class="island-header">
            <div class="island-title"><i class="ri-code-s-slash-line"></i> Schema Markup</div>
            <button class="btn-copy" id="btnCopySchema" aria-label="Copiar Schema">
              <i class="ri-file-copy-line"></i>
            </button>
          </div>
          <pre id="resAnalyzeSchema" style="background: #111827; color: #e5e7eb; padding: 12px; border-radius: 6px; font-size: 11px; overflow-x: auto; margin-top: 6px;"><code>${analy.schema_markup ? JSON.stringify(analy.schema_markup, null, 2) : "{}"}</code></pre>
        </div>
  ...
  if (btnCopySchema) {
    btnCopySchema.addEventListener("click", () => {
      const text = document.getElementById("resAnalyzeSchema")?.innerText || "";
      navigator.clipboard.writeText(text);
      const icon = btnCopySchema.querySelector("i");
      if (icon) {
        icon.className = "ri-check-line";
...
function pdfListHtml(items) {
  return items.length
    ? items
        .map(
          (i) =>
            `<li style="padding:3px 0;border-bottom:1px solid #f3f4f6;font-size:11px;color:#374151">${i}</li>`,
        )
        .join("")
...
function bindAnalyzeExport() {
  ...
  btn.addEventListener("click", async () => {
    ...
    try {
      ...
      const fortaleza = getText("resAnalyzeFortaleza");
      const oportunidad = getText("resAnalyzeOportunidad");
      const h1 = getText("resAnalyzeH1");
      const meta = getText("resAnalyzeMeta");
      const h2List = getList("resAnalyzeH2");
      const kwList = [
        ...document.querySelectorAll("#resAnalyzeKeywords span"),
      ].map((s) => s.innerText.trim());
      const linksList = getList("resAnalyzeLinks");
      const competitors = [
        ...document.querySelectorAll(".analyze-competitor-card"),
      ].map((card) => ({
        rank: card.querySelector(".analyze-rank")?.innerText.trim() || "",
        title:
          card.querySelector(".analyze-competitor-title")?.innerText.trim() ||
          "Sin título",
        url:
          card.querySelector(".analyze-competitor-url")?.innerText.trim() || "",
        meta:
          card
            .querySelector(".analyze-competitor-meta")
            ?.innerText.replace(/^Meta:\s*/, "")
            .trim() || "",
        preview:
          card
            .querySelector(".analyze-competitor-snippet")
            ?.innerText.replace(/^Preview:\s*/, "")
            .trim() || "",
      }));
      const schemaMarkup =
        document.getElementById("resAnalyzeSchema")?.innerText.trim() || "{}";
      const now = new Date().toLocaleDateString("es-ES", {
        year: "numeric",
        month: "long",
        day: "numeric",
      ...
      container.innerHTML = `<div style="padding:32px 36px">
        ...
        <div style="display:flex;gap:12px;margin-bottom:12px">
          ${pdfSection("#6366f1", "Fortalezas (Competencia)", fortaleza)}
          ${pdfSection("#f59e0b", "Oportunidades", oportunidad)}
        </div>
        
        <div style="display:flex;gap:12px;margin-bottom:12px">
          ${pdfSection("#10b981", "H1 Sugerido", h1)}
          ${pdfSection("#8b5cf6", "Meta Description Sugerida", meta)}
        </div>

        <div style="display:flex;gap:12px;margin-bottom:16px">
          <div style="flex:1;background:#f9fafb;border-radius:8px;padding:14px;border:1px solid #e5e7eb;">
            <div style="font-size:9px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:#ec4899;margin-bottom:8px">Keywords Target (${kwList.length})</div>
            <div style="display:flex;flex-wrap:wrap;gap:4px;">
              ${kwList.map((k) => `<span style="background:#fce7f3;color:#be185d;padding:2px 6px;border-radius:4px;font-size:10px">${k}</span>`).join("")}
            </div>
          </div>
          <div style="flex:1;background:#f9fafb;border-radius:8px;padding:14px;border:1px solid #e5e7eb;">
            <div style="font-size:9px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:#14b8a6;margin-bottom:8px">Internal Links (${linksList.length})</div>
            <ul style="margin:0;padding-left:18px">${linksList.map((l) => `<li style="font-size:11px;color:#374151;padding:2px 0">${l}</li>`).join("")}</ul>
          </div>
        </div>
        ...
        <div style="background:#111827;border-radius:8px;padding:14px;margin-bottom:16px">
          <div style="font-size:9px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:#e5e7eb;margin-bottom:8px">Schema Markup Recomendado</div>
          <pre style="margin:0;color:#9ca3af;font-size:10px;white-space:pre-wrap;word-wrap:break-word;"><code>${schemaMarkup}</code></pre>
        </div>
        ...
        <div style="margin-bottom:12px">
          ...
          ${competitors
            ...
            <div style="background:#f9fafb;border-radius:8px;padding:12px;border:1px solid #e5e7eb;margin-bottom:8px">
              ...
              ${c.meta ? `<div style="font-size:10px;color:#4b5563;margin-bottom:2px"><strong>Meta:</strong> ${c.meta}</div>` : ""}
              ${c.preview ? `<div style="font-size:10px;color:#6b7280"><strong>Preview:</strong> ${c.preview}</div>` : ""}
            </div>`,
            )
            .join("")}
        </div>
...
async function fetchHistoryPage({ append = false } = {}) {
  ...
  try {
    const response = await fetch(
      `../../backend/api/seo/history.php?user_id=${encodeURIComponent(session.id)}&limit=${HISTORY_PAGE_SIZE}&offset=${historyOffset}`,
    );
    ...
    const nextOffset =
      Number(data.pagination?.offset ?? historyOffset) +
      Number(data.pagination?.limit ?? HISTORY_PAGE_SIZE);
    historyOffset = Number.isNaN(nextOffset)
